...
mainMenuBtn.onclick = () => { stopExamTimer(); hideAll(); show(modeSelect); };
...
function startQuiz(set) {
  MODE = "quiz";
  selected = set; idx = 0; score = 0; userAnswers = new Array(set.length).fill(null); flagged = [];
  hideAll(); show(quizEl);
...
function hideAll() {
  hide(modeSelect); hide(setupEl); hide(resultsEl); hide(reviewEl); hide(searchEl);
}
...
// ==========================
function toggleFlag() {
  if (flagged.includes(idx)) flagged = flagged.filter(i => i !== idx);
...
nextBtn.onclick = () => {
  if (idx < selected.length - 1) { idx++; renderQuestion(); }
  else finishQuiz();
};
...
function finishQuiz() {
  ...
  finalScore.innerHTML = `
    ...
    <span class="result-badge ${pct >= 75 ? "result-pass" : "result-fail"}">
      ${pct >= 75 ? "PASS" : "FAIL"} — Pass mark ${PASS_MARK}%
    </span>
  `;
...
function renderSearch(list) {
  ...
  list.forEach((q, i) => {
    ...
    item.onclick = () => {
      ansDiv.style.display = ansDiv.style.display === "none" ? "grid" : "none";
    };
